    ...
    let promise = new Promise(async(resolve, reject) => {
        ...
        if (type === "request") {
            ...
            if (typeDetails.type === "orgJoin") {
                notif.meta.description = `${user.given_name} has requested to join '${org.name}' club`
            } else if (typeDetails.type === "eventOrgHost") {
                let event = await Event.findOne({ _id: typeDetails.eventID })
                notif.meta.description = `${user.given_name} is requesting '${org.name}' to host '${event.name}' event`
            }
            ...
            if (typeDetails.type === "project") {
                ...
                notif.meta.description = `${user.given_name} has commented on your project '${project.title}'`
                ...
                notif.meta.description = `${user.given_name} has commented on your post '${post.title}'`
                notif.meta.img = user.pfp_url
